...
public interface AssignedAssetsRepo extends JpaRepository<AssignedAssets, Long> {
	...
	@Query(value="SELECT tas.*,GROUP_CONCAT(ta.asset_name),GROUP_CONCAT(at.type_name),te.emp_name,te.emp_email,te.emp_contact,te.desig_id,des.desig_name,td.dept_id,td.dept_name,td.comp_id,tc.comp_name FROM tbl_assigned_assets tas JOIN tbl_assets ta ON ta.asset_id=tas.asset_id JOIN tbl_assettype AS at ON at.type_id=ta.type_id JOIN tbl_employee AS te ON te.emp_id=tas.emp_id JOIN tbl_designation AS des ON des.desig_id=te.desig_id JOIN tbl_department AS td ON td.dept_id=te.dept_id JOIN tbl_company AS tc ON tc.comp_id=td.comp_id",nativeQuery = true)
	//public List<AssignedAssets[]> getAllNewAssignedAssets();
