...
public interface AssignedAssetsRepo extends JpaRepository<AssignedAssets, Long> {
	...
	@Query(value="tas.*,ta.*,at.*,te.*,des.*,td.*,tc.*,GROUP_CONCAT(ta.asset_name) AS assigned_assets,GROUP_CONCAT(at.type_name) AS assigned_asset_types FROM tbl_assigned_assets  AS tas JOIN tbl_assets AS ta ON ta.asset_id=tas.asset_id  JOIN tbl_employee AS te ON te.emp_id=tas.emp_id JOIN tbl_assettype AS at ON at.type_id=ta.type_id JOIN tbl_designation AS des ON des.desig_id=te.desig_id JOIN tbl_department AS td ON td.dept_id=te.dept_id JOIN tbl_company AS tc ON tc.comp_id=td.comp_id GROUP BY tas.emp_id",nativeQuery = true)
